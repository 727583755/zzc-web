...
<html>
	...
	<body>
		    <div class="container">
				<div class="row">
					<button type="button" class="btn btn-success" onclick="location='${WEB_ROOT_PATH}/admin/musicalbum/add'">新增</button>
				</div>
			</div>

			<div class="container">
				<div class="row">
					<form class="form-inline" action="${WEB_ROOT_PATH}/admin/musicalbum" method="post" id="myForm">
			...
			<div class="container">
						...
						<tbody>
							<#list data.result as p>
								<tr>
									...
									<td>${p.hot}</td>
									<td>
										<a target="_blank" href="${WEB_ROOT_PATH}/admin/musicalbum/${p.id}?p=${data.pageNum}" ><button type="button" class="btn btn-primary" >修改</button></a>
										<a target="_blank" href="${WEB_ROOT_PATH}/admin/musicalbum/${p.id}/addMusic?p=${data.pageNum}" ><button type="button" class="btn btn-success" >增加乐曲</button></a>
				...
				<@pager url="${WEB_ROOT_PATH}/admin/musicalbum" data=data/>
	...
	<script>
	function collapse(who, albumId) {
		$(who).collapse('toggle');
		var tboty = $("#info_" + albumId);
		$.ajax({
        	url: "${WEB_ROOT_PATH}/admin/musicalbum/getMusicByMusicAlbumId",
            dataType : "json",
            type: "post",
            data: {
                albumId: albumId
            ...
            success: function(data) {
                ...
                for(var i=0; i<data.length; i = i+1){
                	...
                	tr += "<td>" + (d.size==null?"":d.size) + "</td>";
                	tr += "<td>" + (d.duration==null?"":d.duration) + "</td>";
                	tr += "<td>" + (d.announcerName==null?"":d.announcerName) + "</td>";
                	tr += '<td><button type="button" class="btn btn-primary" onclick="redirect(' + d.id + ')">修改</button><button type="button" class="btn btn-danger" onclick="deleteMusic(' + d.id + ')">删除</button></td>';
                	tr += "</tr>";
                	tboty.append(tr);
                }
            }
	...
	function redirect(id) {
		window.open('${WEB_ROOT_PATH}/admin/music/' + id, '_blank');
	}
	...
	function deleteMusic(musicId) {
        $.ajax({
            url: "${WEB_ROOT_PATH}/admin/music/deleteMusic",
            dataType : "json",
            type: "post",
            data: {
                musicId:musicId
	...
	function changeStatus(id, who) {
		...
		$.ajax({
        	url: "${WEB_ROOT_PATH}/admin/musicalbum/changeStatus",
            dataType : "json",
            type: "post",
            data: {
                id: id,
